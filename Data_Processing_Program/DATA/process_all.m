% % % ear spO2 script bioboard harry davies 20/05/2021
clear all
close all
...
%     fclose(fid);
    ...
    if Number_of_LEDs == 2
        ...
        title('heart rate estimate on chest');
    ...
    %ppgTimeStampVectorSeconds_ear_raw = (ppgTimeStampVector_ear_raw/1000)*32;
    ppgTimeStampVectorSeconds_ear_raw = ((1:length(ir_ear_raw))/1000)*32;
    %% process PPG data
    trig_s = 0;
...
%     
    ...
    [ir_ear_pks_max, ir_ear_locs_max, ir_ear_widths_max, ir_ear_proms_max] = findpeaks(ir_ear_filt,'MinPeakDistance',10,'MinPeakProminence',prominence1);
    [ir_ear_pks_min, ir_ear_locs_min, ir_ear_widths_min, ir_ear_proms_min] = findpeaks(movmean(-ir_ear_filt,1),'MinPeakDistance',10,'MinPeakProminence',prominence1);
...
%     end
    ...
    prominence2 = 30;
    [red_ear_pks_max, red_ear_locs_max,red_ear_widths,red_ear_proms] = findpeaks(movmean(red_ear_filt,1),'MinPeakDistance',10,'MinPeakProminence',prominence2);
    [red_ear_pks_min, red_ear_locs_min] = findpeaks(movmean(-red_ear_filt,1),'MinPeakDistance',10,'MinPeakProminence',prominence2);
...
%        figure; plot(red_ear_pks_max);
    ...
    title('PPG singnal by IR and red on chest');
    ...
    ref = '.';
    seconds_start = 0;
    if strcmp('sub1alt2',ref) == 1
       seconds_start = 540;
       spo2_fin = [87 88 89 89 86 87 86 87 86 87];
       time_fin = [21 81 141 201 261 321 381 441 501 561];
    end
    
    if strcmp('sub1alt1',ref) == 1
       seconds_start = 210;
       spo2_fin = [90 89 89 90 89 89 90 88 88 88 89 88 87 89 88 87 87 86 87 87];
       time_fin = [21 81 141 201 261 321 381 441 501 561 621 681 741 801 861 921 981 1041 1101 1161];
    end
    
    
    if strcmp('sub2alt1',ref) == 1
       seconds_start = 160;
       spo2_fin = [94 93 93 94 92 93 92 92 92 93 91 92 93 91 91 91 90 91 92 92 92];
       time_fin = [21 81 141 201 261 321 381 441 501 561 621 681 741 801 861 921 981 1041 1101 1161 1221];
    end
    
    
    figure;
    hold on
    plot(ppgTimeStampVectorSeconds_ear(1:end-1)-seconds_start,medfilt1(spo2_ear_final,1000),'LineWidth',1,'Color',[1 0 0]);
...
a = readtable('eeg.csv');  % skips the first three rows of data
eeg_table = table2array(a);
...
% plot(eeg_table(:,3));
%%
electrical_sig = eeg_table(:,3);
electrical_time = (1:length(electrical_sig))/500;
figure;
plot(electrical_time,electrical_sig);
ecg_filt = bandpass(detrend(electrical_sig),[0.5, 40],500);
figure;
plot(ecg_filt);
